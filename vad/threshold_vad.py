"""
Simple Threshold-based Voice Activity Detection (VAD)
"""
...
class ThresholdVAD:
    ...
    def process_audio(self, audio_data: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        for i in range(num_frames):
            ...
            vad_mask[i] = 1.0 if energy > self.threshold else 0.0
    ...
    def is_voice_activity(self, audio_data: np.ndarray) -> Tuple[bool, str]:
        ...
        if is_voice:
            reason = f"Voice detected (ratio: {voice_ratio:.2f}, threshold: {self.threshold})"
        else:
            reason = f"No voice detected (ratio: {voice_ratio:.2f}, threshold: {self.threshold})"
        
        return is_voice, reason
    
    def reset(self):
        """Reset VAD state (no state to reset for threshold VAD)"""
        pass
